...
# Let's find where generate_dossiers starts and rewrite it with proper 4-space indentation
pos = text.find('def generate_dossiers(player_timelines):')
if pos == -1:
    print("generate_dossiers not found")
else:
    # Let's keep everything before generate_dossiers
    prefix = text[:pos]
...
'''
    with open('battle_analytics/generate_personal_stats.py', 'w', encoding='utf-8') as f:
        f.write(prefix + new_generate_dossiers)
    print("Successfully rewritten generate_dossiers with clean indentation!")
