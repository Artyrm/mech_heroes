...
# Чистая база игроков без Prostou
opponents_data = {
    ...
    'Tranduil': {'rating': 4215, 'winrate': 1.0, 'unlimited': False}, # 100% всегда доступен
    'Semen 444': {'rating': 4193, 'winrate': 0.70, 'unlimited': False}, # 70% всегда доступен
    ...
    'Mmv': {'rating': 4156, 'winrate': 0.88, 'unlimited': True}, # 88% NPC
    'Hunter': {'rating': 4150, 'winrate': 0.875, 'unlimited': True}, # 87.5% NPC
    'Nastoyki': {'rating': 4147, 'winrate': 0.833, 'unlimited': True}, # 83.3% NPC
...
def run_simulation(total_battles=100, num_sims=5000):
    ...
    # Резервы со 100% винрейтом
    preserves = {'Tranduil', 'Semen 444', 'RaRik'}
    
    total_fought = {}
    total_won = {}
    demo_log = []
    
    print(f"\n[1/2] Запуск симуляции сбалансированного фермерства (Севооборот, {total_battles} боёв)...")
    ...
    for sim in range(num_sims):
        ...
        for step_idx in range(total_battles):
            ...
            # Условия разблокировки резервов
            ...
            # ДИВЕРСИФИКАЦИЯ: собираем пул кандидатов, чье EV отстает от лидера не более чем на 1.5 очка.
            # Это заставляет алгоритм чередовать бои против Сэма, Mmv, Hunter, Настроек и т.д.
    ...
    print("\n" + "="*95)
    print(" ДЕМОНСТРАЦИОННЫЙ ХОД ШТУРМА (СБАЛАНСИРОВАННЫЙ СЕВООБОРОТ - 100 БИЛЕТОВ)")
    print("="*95)
    print(f"{'Бой':<4} | {'Противник':<15} | {'Исход':<6} | {'Изменение':<10} | {'Ваш ELO':<8} | {'ELO Врага':<8} | {'Просадка врага':<12}")
    print("-"*95)
    
    for row in demo_log[:20]: # Фаза 1: Сбалансированный разгон по разным NPC
        sign = "+" if row['Change'] >= 0 else ""
        print(f"{row['Battle']:<4} | {row['Opponent']:<15} | {row['Outcome']:<6} | {sign}{row['Change']:<8} | {row['Player ELO']:<8} | {row['Opp ELO']:<8} | -{row['Opp Sunk']:.1f}")
    print("...")
    for row in demo_log[50:65]: # Фаза 2: Клинч
        sign = "+" if row['Change'] >= 0 else ""
        print(f"{row['Battle']:<4} | {row['Opponent']:<15} | {row['Outcome']:<6} | {sign}{row['Change']:<8} | {row['Player ELO']:<8} | {row['Opp ELO']:<8} | -{row['Opp Sunk']:.1f}")
    print("...")
    for row in demo_log[-15:]: # Фаза 3: Разгон резервами
        sign = "+" if row['Change'] >= 0 else ""
        print(f"{row['Battle']:<4} | {row['Opponent']:<15} | {row['Outcome']:<6} | {sign}{row['Change']:<8} | {row['Player ELO']:<8} | {row['Opp ELO']:<8} | -{row['Opp Sunk']:.1f}")
    print("="*95)
