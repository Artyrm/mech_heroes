...
# Использование аргументов командной строки: имя и ID
if len(sys.argv) < 3:
    print("Использование: python activity_analyzer.py <ИМЯ> <ID>")
    sys.exit(1)
...
# Базовые пути
BASE_DIR = r"G:\Video\!Медведи\Mech Heroes\Клан Орки\accountant_bot"
PATHS = {
    "online_history": os.path.join(BASE_DIR, "arena", "squads", USER_ID, "online_history.json"),
    "battles_dir": os.path.join(BASE_DIR, "battle_analytics", USER_NAME),
    "holidays": os.path.join(BASE_DIR, "arena", "graphs", "holidays.json"),
    "graphs_dir": os.path.join(BASE_DIR, "arena", "graphs")
}
...
def generate_graph():
    ...
    # Выходные/праздники (поверх активности с прозрачностью, zorder=2)
    holidays = load_holidays()
    for i, d in enumerate(all_dates):
        if d.weekday() >= 5 or d.strftime("%Y-%m-%d") in holidays:
            ax.add_patch(patches.Rectangle((i, 0), 1, 24, facecolor='salmon', alpha=0.4, zorder=2))
    ...
    plt.title(f'Активность {USER_NAME} (Синий - любая активность, Розовый - выходные)', fontsize=14, pad=20)
    plt.tight_layout()

    out_path = os.path.join(PATHS["graphs_dir"], f"{USER_NAME}_activity.png")
