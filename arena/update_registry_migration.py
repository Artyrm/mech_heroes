...
migration_code = '''def register_user(uid, nickname):
...
new_text = re.sub(r'def register_user\(uid, nickname\):.*?(?=\ndef |\Z)', migration_code, text, flags=re.DOTALL)
if new_text != text:
    with open(path, 'w', encoding='utf-8') as f:
        f.write(new_text)
    print("Successfully updated registry_manager.py with migration logic via python script!")
else:
